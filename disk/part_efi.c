		      ...
		      disk_partition_t *info)
{
    ...
    /* Previous dev_desc info backup */
    static enum if_type prev_dev_desc_if_type = IF_TYPE_UNKNOWN;
    static int prev_dev_desc_devnum = -1;
    static lbaint_t prev_dev_desc_lba = 0;
    ...
    */
	if (prev_dev_desc_if_type != dev_desc->if_type || prev_dev_desc_devnum != dev_desc->devnum || prev_dev_desc_lba != dev_desc->lba) {
		if (gpt_pte) {
			free(gpt_pte);
            printf("%s-%d: last_usable_lba(0x%llx)+0x22(0x%llx) != dev_desc->lba(0x%lx), free gpt_pte!\n",
                    __FUNCTION__, __LINE__,
                    gpt_head->last_usable_lba, gpt_head->last_usable_lba + 0x22, dev_desc->lba);
        }
		gpt_pte = NULL;
        prev_dev_desc_if_type = dev_desc->if_type;
        prev_dev_desc_devnum = dev_desc->devnum;
        prev_dev_desc_lba = dev_desc->lba;
	}
