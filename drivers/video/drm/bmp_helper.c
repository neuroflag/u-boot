...
int logo_rotation(void *newbmp, void *beforebmp, int width, int height, int byte, int rotation)
{
    unsigned char *dst, *imgdata;
    int padded_width, beforebmp_size;
	padded_width = height & 0x3 ? (height & ~0x3) + 4 : height;
    beforebmp_size = height * width * byte;
    int ret = rotation;
    ...
    switch(rotation) {
        case FIREFIY_LOGO_ROTATION_90:
            ...
            break;
        case FIREFIY_LOGO_ROTATION_180:
            for(int i = 0; i < height; i++){
                for(int j = 0; j < width; j++){
            ...
            break;
        case FIREFIY_LOGO_ROTATION_270:
            for(int i = 0; i < height; i++){
                for(int j = 0; j < width; j++){
