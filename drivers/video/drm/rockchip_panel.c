...
static void panel_simple_prepare(struct rockchip_panel *panel)
{
	...
	if (dm_gpio_is_valid(&priv->reset_tc_gpio)){
		dm_gpio_set_value(&priv->reset_tc_gpio, 1);
	}
...
static void panel_simple_unprepare(struct rockchip_panel *panel)
{
	...
	// for tc358775
	if (dm_gpio_is_valid(&priv->reset_tc_gpio))
		dm_gpio_set_value(&priv->reset_tc_gpio, 0);
	if (dm_gpio_is_valid(&priv->enable_tc_gpio)){
...
static int rockchip_panel_probe(struct udevice *dev)
{
	...
	ret = gpio_request_by_name(dev, "reset-tc-gpios", 0,
				   &priv->reset_tc_gpio, GPIOD_IS_OUT);
	if (ret && ret != -ENOENT) {
		printf("%s: Cannot get reset tc GPIO: %d\n", __func__, ret);
	}
