...
static int load_bmp_logo(struct logo_info *logo, const char *bmp_name)
{
    ...
    if(logo->rotation)
    {
        ...
        if((logo->rotation == FIREFIY_LOGO_ROTATION_90) || (logo->rotation == FIREFIY_LOGO_ROTATION_270)){
            logo->width = newW;
            logo->height = newH;
        }
        rotation = logo_rotation(dst, dst_temp, width, height, byte, logo->rotation);
        if(rotation < 0){
...
static int rockchip_display_probe(struct udevice *dev)
{
	...
	ofnode_for_each_subnode(node, route_node) {
        ...
        switch(rotation){
        case 90:
            s->logo_rotation = FIREFIY_LOGO_ROTATION_90;
            break;
        case 180:
            s->logo_rotation = FIREFIY_LOGO_ROTATION_180;
            break;
        case 270:
            s->logo_rotation = FIREFIY_LOGO_ROTATION_270;
            break;
        default:
            s->logo_rotation = FIREFIY_LOGO_ROTATION_0;
            break;
        }
