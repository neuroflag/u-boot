...
static int rkkey_parse_recoverykey_dt(const void *blob, int *key_max_value, int *key_min_value)
{
	int recoverykey_node;

	recoverykey_node = fdt_node_offset_by_compatible(blob, -1, "adc-keys");
	if (recoverykey_node < 0) {
		printf("no key node\n");
		return -1;
	}

	recoverykey_node = fdt_subnode_offset(blob, recoverykey_node, "recovery-key");
	if (recoverykey_node < 0) {
		printf("no recovery key node\n");
		return -1;
	}

	*key_min_value = fdtdec_get_int(blob, recoverykey_node, "keyvalue-low", 0);
	*key_max_value= fdtdec_get_int(blob, recoverykey_node, "keyvalue-high", 30);
...
//#elif defined(CONFIG_ADC)
	...
	int adc_min_value = KEY_DOWN_MIN_VAL, adc_max_value = KEY_DOWN_MAX_VAL;
	u32 val, chns[2];
	...
	rkkey_parse_recoverykey_dt(blob, &adc_max_value, &adc_min_value);
	//printf("lzf debug %d %d %d in [%s-%d] \r\n", val,adc_max_value, adc_min_value,__func__,__LINE__);
	return ((val >= adc_min_value) && (val <= adc_max_value));
//#endif
